...
interface NodeItem {
	...
	pulseSpeed: number;
	colorType: number;
	isConnected?: boolean; // Added for proximity glow
}
...
export const AbstractCorporateNetwork: React.FC = () => {
	...
	const animatedNodesBase = useMemo(() => { // Renamed to animatedNodesBase
	...
	const cameraPanX = interpolate(Math.sin(loopAngle * 0.5), [-1, 1], [-50, 50]);
	const cameraPanY = interpolate(Math.cos(loopAngle * 0.7), [-1, 1], [-40, 40]);
	const cameraZoom = interpolate(Math.sin(loopAngle * 0.3), [-1, 1], [1, 1.05]);
	...
	return (
		<AbsoluteFill style={{
			...
			{/* Wrapper for dynamic camera movement */}
			<div style={{
				position: 'absolute',
				width: '100%',
				height: '100%',
				...
				<svg style={{
					...
					{connections.map((conn, idx) => {
						...
						// Menghitung titik kontrol untuk garis lengkung (Bezier)
						const midX = (conn.from.x + conn.to.x) / 2 + (idx % 2 === 0 ? 40 : -40);
						const midY = (conn.from.y + conn.to.y) / 2 + (idx % 2 === 0 ? -40 : 40);
						const pathData = `M ${conn.from.x} ${conn.from.y} Q ${midX} ${midY} ${conn.to.x} ${conn.to.y}`;
				...
				{animatedNodes.map((node) => { // Use the updated animatedNodes with isConnected status
					...
					return (
						...
						>
							...
							{/* Node Proximity Glow (if connected) */}
							{node.isConnected && (
								<div style={{
									position: 'absolute',
									top: '0%',
									...
									opacity: interpolate(
										Math.sin(loopAngle * 5 + node.id), // Faster pulse for connected nodes
										[-1, 1],
										[0.1, 0.3]
									),
									filter: 'blur(15px)',
									transform: `scale(${interpolate(Math.sin(loopAngle * 5 + node.id), [-1, 1], [1, 1.1])})`,
								}} />
							)}
							...
							{isHighlighted && (
								<div style={{
									...
									<span style={{ color: baseColor }}>●</span> {`N_0${node.id}`}
									<span style={{ marginLeft: 8, fontSize: '10px', opacity: 0.6 }}>
										{`${(random(`lat-${node.id}-${Math.floor(frame / 10)}`) * 20).toFixed(1)}ms`}
									</span>
								</div>
							)}
						</div>
			...
			</div> {/* End of camera movement wrapper */}
